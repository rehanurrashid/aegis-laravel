...
class DocumentSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // ── Runtime plan ID lookup — never hardcode plan IDs ───────────────────
        $planSarah = ContinuityPlan::where('practitioner_id', 'p_sarah')
            ->orderByDesc('created_at')->value('id');
        $planMaria = ContinuityPlan::where('practitioner_id', 'p_maria')
            ->orderByDesc('created_at')->value('id');
        $planDavid = ContinuityPlan::where('practitioner_id', 'p_david')
            ->orderByDesc('created_at')->value('id');

        // ── continuity_documents ───────────────────────────────────────────────
        $documents = [
            // p_sarah — Fully executed MSA
            [
            ...
            // p_sarah — Pending provider signature
            [
                'id'               => 'doc_sarah_baa',
                'plan_id'          => $planSarah,
                'practitioner_id'  => 'p_sarah',
            ...
            // p_sarah — Awaiting CS countersignature
            [
                'id'               => 'doc_sarah_sow',
                'plan_id'          => $planSarah,
                'practitioner_id'  => 'p_sarah',
            ...
            // p_sarah — CS Engagement Agreement (fully executed — satisfies Plan Section 7)
            [
        ...
        foreach ($documents as $doc) {
            // Skip if plan lookup failed (practitioner has no plan yet)
            if (empty($doc['plan_id'])) {
                continue;
            }
        ...
        DB::table('document_signatures')
            ->whereIn('document_id', array_column($documents, 'id'))
            ->delete();
