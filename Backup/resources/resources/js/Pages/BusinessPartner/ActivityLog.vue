<template>
  <AppLayout :user="user" portal="business_partner" activePage="activity" pageTitle="Activity Log">

    <!-- ═══ HEADER ═══ -->
    <div class="al-header">
      <div>
        <div class="al-eyebrow">Business Partner Portal</div>
        <h1 class="al-title">Activity Log</h1>
        <p class="al-sub">Contracts, milestones, financial events and communications</p>
      </div>
    ...
    <div class="al-filters">
      ...
      <select v-model="category" class="form-select al-select"><option>All Categories</option><option>Contracts</option><option>Financial</option><option>Milestones</option><option>Messages</option></select>
    ...
    <div class="al-tabs">
      <button v-for="t in tabs" :key="t.key" type="button" class="al-tab" :class="{ active: activeTab === t.key }" @click="activeTab = t.key">
        ...
        {{ t.label }}
        <span v-if="t.count" class="al-tab-count">{{ t.count }}</span>
      </button>
    </div>

    <!-- ═══ FEED ═══ -->
    <template v-for="g in groups" :key="g.key">
      <template v-if="g.events.length">
        <div class="al-group-label">{{ g.label }} <span class="al-group-count">{{ g.events.length }} events</span></div>
        <div class="al-feed">
          <div v-for="ev in g.events" :key="ev.hash" class="al-event" :class="'rail-' + ev.rail" role="button" tabindex="0" @click="openEvent(ev)" @keyup.enter="openEvent(ev)">
            <div class="al-icon ico-document" v-html="docIcon"></div>
            <div class="al-body">
              ...
              <div class="al-event-meta">
                <span class="al-badge badge-document">Document</span>
    ...
    <Modal v-model="showDetail" :title="selectedEvent ? selectedEvent.title : ''">
      <template v-if="selectedEvent">
        <div class="al-detail">
          <div class="al-detail-row"><span class="al-detail-label">Date &amp; Time</span><span class="al-detail-val">Jun 22, 2026, {{ selectedEvent.time }}</span></div>
          <div class="al-detail-row"><span class="al-detail-label">Category</span><span class="al-detail-val">Document</span></div>
          <div class="al-detail-row"><span class="al-detail-label">Performed By</span><span class="al-detail-val">Acme Practice Services (You)</span></div>
...
const totalEvents = 29;

const sa = 'fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"';
const docIcon = `<svg viewBox="0 0 24 24" ${sa}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>`;
const fileIcon = `<svg viewBox="0 0 24 24" ${sa}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>`;
const cardIcon = `<svg viewBox="0 0 24 24" ${sa}><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg>`;
const targetIcon = `<svg viewBox="0 0 24 24" ${sa}><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>`;
const chatIcon = `<svg viewBox="0 0 24 24" ${sa}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>`;

const events = [
  // ── TODAY ──
  { title: 'Job saved: Substance Use Practice – Revenue Cycle Management', desc: 'You bookmarked this job for later.', tab: 'contracts', hash: 'C9910AC4-E78', time: '10:00 AM', day: 'today', rail: 'none', unread: true },
  { title: 'Invoice issued: INV-2026-020 — Elena Vasquez', desc: 'You issued an invoice for EHR Migration & Training.', tab: 'financial', hash: 'A4E7834D-9A8', time: '9:30 AM', day: 'today', rail: 'none', unread: true },
  // ── EARLIER ──
  { title: 'Proposal withdrawn: EHR Setup & Staff Training', desc: 'You withdrew your proposal and Elena Vasquez was notified.', tab: 'contracts', hash: '9372A5F6-142', time: '1:00 PM', day: 'earlier', rail: 'gold', unread: true },
  { title: 'Proposal declined: Practice Website Redesign & SEO', desc: 'Dr. Anna Thompson selected another partner whose timeline started sooner.', tab: 'contracts', hash: '72F020B0-342', time: '6:00 PM', day: 'earlier', rail: 'none', unread: true },
  { title: 'Job saved: HIPAA Compliance Audit', desc: 'You bookmarked this job for later review.', tab: 'contracts', hash: 'F64B9C20-6FA', time: '4:22 PM', day: 'earlier', rail: 'none', unread: true },
  { title: 'Proposal submitted: EHR Setup & Staff Training', desc: 'Your proposal has been sent to Elena Vasquez.', tab: 'contracts', hash: '9DA67F36-4B9', time: '10:00 AM', day: 'earlier', rail: 'none', unread: true },
  { title: 'Job saved: HIPAA-Compliant IT Support Engineer', desc: 'You bookmarked this job for later review.', tab: 'contracts', hash: 'D563D604-9B4', time: '11:45 AM', day: 'earlier', rail: 'none', unread: true },
  { title: 'Job saved: Medical Billing Specialist — Psychiatry Practice', desc: 'You bookmarked this job for later review.', tab: 'contracts', hash: '6585CCD3-BB8', time: '11:18 AM', day: 'earlier', rail: 'none', unread: true },
  { title: 'Job viewed: Medical Billing Specialist — Psychiatry Practice', desc: 'You browsed this open listing from Dr. Sarah Johnson.', tab: 'contracts', hash: '1F067FC0-995', time: '11:15 AM', day: 'earlier', rail: 'none', unread: true },
  { title: 'Proposal submitted: Practice Website Redesign & SEO', desc: 'Your proposal has been sent to Dr. Anna Thompson.', tab: 'contracts', hash: '99D8E571-ECF', time: '12:00 PM', day: 'earlier', rail: 'none', unread: true },
  { title: 'Contract signed', desc: 'Service contract executed with a practitioner client.', tab: 'contracts', hash: 'C27C7C51-B69', time: '5:20 PM', day: 'earlier', rail: 'gold', unread: true },
  { title: 'Proposal submitted', desc: 'Submitted proposal for a practitioner job posting.', tab: 'contracts', hash: '6FCB3C04-887', time: '12:00 PM', day: 'earlier', rail: 'none', unread: true },
  { title: 'Proposal accepted: Virtual Administrative Assistant', desc: 'Dr. Sarah Johnson accepted your proposal. A contract is being prepared.', tab: 'contracts', hash: 'A1A8EDC2-5E4', time: '4:40 PM', day: 'earlier', rail: 'none', unread: true },
  { title: 'Milestone submitted: Phase 3 — Migration, training, & launch', desc: 'You submitted work for review to Dr. Sarah Johnson.', tab: 'milestones', hash: 'B7C219AE-44D', time: '3:00 PM', day: 'earlier', rail: 'none', unread: false },
  { title: 'Milestone approved: Phase 2 — Billing & Stripe integration', desc: 'Dr. Sarah Johnson approved your milestone. Invoice generated.', tab: 'milestones', hash: 'E1F8B0C7-2A9', time: '9:00 AM', day: 'earlier', rail: 'none', unread: false },
  { title: 'Invoice paid: INV-2026-018 — Dr. Sarah Johnson', desc: '$1,642.20 deposited to your Stripe account.', tab: 'financial', hash: 'D31C77A0-5E2', time: '8:15 AM', day: 'earlier', rail: 'none', unread: false },
  { title: 'Message received: Dr. Sarah Johnson', desc: 'New message regarding the billing retainer contract.', tab: 'messages', hash: 'F0A2D6B1-77C', time: '7:50 AM', day: 'earlier', rail: 'none', unread: false },
  { title: 'Contract signed: EHR Migration & Training', desc: 'Service contract executed with Elena Vasquez.', tab: 'contracts', hash: 'C5E90B23-118', time: '6:30 PM', day: 'earlier', rail: 'none', unread: false },
  { title: 'Message sent: Elena Vasquez', desc: 'You replied about the EHR migration timeline.', tab: 'messages', hash: '0B7E4F2C-93A', time: '5:05 PM', day: 'earlier', rail: 'none', unread: false },
  { title: 'Job viewed: Patient Intake Automation & CRM Setup', desc: 'You browsed this open listing from Dr. Thomas Nguyen.', tab: 'contracts', hash: '2D4A8E70-6B1', time: '2:40 PM', day: 'earlier', rail: 'none', unread: false },
];

const tabs = [
  { key: 'all', label: 'All', icon: '', count: 29 },
  { key: 'contracts', label: 'Contracts', icon: fileIcon, count: 12 },
  { key: 'financial', label: 'Financial', icon: cardIcon, count: 3 },
  { key: 'milestones', label: 'Milestones', icon: targetIcon, count: 0 },
  { key: 'messages', label: 'Messages', icon: chatIcon, count: 0 },
];

const catMap = { Contracts: 'contracts', Financial: 'financial', Milestones: 'milestones', Messages: 'messages' };
const visibleEvents = computed(() => {
  let list = events;
  if (activeTab.value !== 'all') list = list.filter(e => e.tab === activeTab.value);
  if (category.value !== 'All Categories') list = list.filter(e => e.tab === catMap[category.value]);
...
const groups = computed(() => [
  { key: 'today', label: 'Today · June 22, 2026', events: visibleEvents.value.filter(e => e.day === 'today') },
  { key: 'earlier', label: 'Earlier', events: visibleEvents.value.filter(e => e.day === 'earlier') },
]);
...
<style scoped>
/* ════════════════════════════════════════════════════════════════
   BP — ACTIVITY LOG (shared CS/SS design)
   ════════════════════════════════════════════════════════════════ */
...
/* TABS */
.al-tabs { display:inline-flex; align-items:center; gap:2px; padding:4px; background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-full); margin-bottom:18px; flex-wrap:wrap; }
...
.al-group-label:not(:first-of-type) { margin-top:24px; }
