<!--
  pages/provider/ContinuityStewards.vue — manage CS designations.
...
<template>
  <AppLayout>
    <AegisHeroBanner
      ...
      subtitle="The clinician who will receive your active clients in an incident."
    >
      <template #actions>
        <button type="button" class="btn btn-primary" @click="openModal('inviteCsModal')">
    ...
    <div class="stat-chips-row">
      <AegisStatChip icon="users-network" :value="active.length"  label="Active Stewards" />
    ...
    <AegisEmptyState
      ...
      icon="users-network"
      title="No Continuity Steward yet"
      description="Invite a colleague who can step in if you're unavailable."
    >
      <template #action>
        <button type="button" class="btn btn-primary" @click="openModal('inviteCsModal')">
          Invite Steward
    ...
    <!-- Invite modal -->
    <AegisModal
      :model-value="isOpen('inviteCsModal').value"
      title="Invite a Continuity Steward"
      @update:model-value="(v) => !v && closeModal('inviteCsModal')"
    >
      ...
      <template #footer>
        <button type="button" class="btn btn-outline" @click="closeModal('inviteCsModal')">Cancel</button>
...
function invite() {
  inviteForm.post(route('provider.continuity-stewards.invite'), {
    preserveScroll: true,
    onSuccess: () => {
      toast.success('Invitation sent.')
      inviteForm.reset()
      closeModal('inviteCsModal')
    },
  })
}
...
function remove(s) {
  ...
  router.delete(route('provider.continuity-stewards.destroy', { steward: s.id }),
    { onSuccess: () => toast.success('Steward removed.') })
}
function resend(s) {
  router.post(route('provider.continuity-stewards.resend', { invitation: s.id }), {}, {
    onSuccess: () => toast.success('Invitation resent.'),
  })
}
function cancel(s) {
  router.delete(route('provider.continuity-stewards.cancel-invite', { invitation: s.id }),
