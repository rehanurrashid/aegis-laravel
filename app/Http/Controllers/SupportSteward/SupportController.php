...
class SupportController extends Controller
{
    ...
    {
        $user = $request->user();
        return Inertia::render('support-steward/Support', [
    ...
    {
        return Inertia::render('support-steward/HelpCenter', [
            'articles' => \App\Models\HelpArticle::where('published', 1)
                ->where(fn($q) => $q->where('role_visibility', 'all')->orWhere('role_visibility', 'support_steward'))
                ->orderBy('sort_order')
                ->get(),
        ]);
    }
