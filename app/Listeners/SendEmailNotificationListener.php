...
class SendEmailNotificationListener
{
    ...
    {
        // Wave 6: Contract is now born in pending_signature (not active).
        // Both parties get the "please sign" email via template 50.
        return [
            ['user_id' => $e->contract->practitioner_id, 'gate_key' => 'notify_payment',
             'template' => 'emails.business.50-contract-pending-signature',
             'data' => ['contract_id' => $e->contract->id, 'recipient_role' => 'provider']],
            ['user_id' => $e->contract->bp_id, 'gate_key' => 'notify_payment',
             'template' => 'emails.business.50-contract-pending-signature',
             'data' => ['contract_id' => $e->contract->id, 'recipient_role' => 'bp']],
        ];
    }

    private function contractSigned(ContractSigned $e): array
    {
        // Wave 6: Use new fully-executed template 51 with PDF link + role-specific copy.
        $base = rtrim(config('app.url'), '/');
        return [
            ['user_id' => $e->contract->practitioner_id, 'gate_key' => 'notify_payment',
             'template' => 'emails.business.51-contract-fully-executed',
             'data' => [
                'contract_id'    => $e->contract->id,
                'recipient_role' => 'provider',
                'pdf_url'        => $base . '/provider/support-services/contracts/' . $e->contract->id . '/pdf',
             ]],
            ['user_id' => $e->contract->bp_id, 'gate_key' => 'notify_payment',
             'template' => 'emails.business.51-contract-fully-executed',
             'data' => [
                'contract_id'    => $e->contract->id,
                'recipient_role' => 'bp',
                'pdf_url'        => $base . '/business/contracts/' . $e->contract->id . '/pdf',
             ]],
        ];
    }
