...
- **`confirmAction()` is CALLBACK-based, never awaited** — signature is `confirmAction(options, callback)`. The gated logic lives inside the callback. Never `const ok = await confirmAction(...)`, never `if (!ok) return`. Example: `confirmAction({ title, message, confirmLabel, destructive: true }, () => deleteForm.delete(route('...')))`.
- **All `useForm()` at setup top-level** — never declare a `useForm()` inside a function, handler, `if` block, or callback (it re-instantiates every call and breaks reactivity). For a many-field config save use ONE form + `.transform(() => ({ ...allFields }))` posting to a single atomic endpoint, not several parallel PUTs.
- **Hydrate reactive config state from a prop, never hardcode** — `const nc = props.xxxConfig || {}` then spread into `reactive({})`. Hardcoded selections silently overwrite saved DB state on every page load.
