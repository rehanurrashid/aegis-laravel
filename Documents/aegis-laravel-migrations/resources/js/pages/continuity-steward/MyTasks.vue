<!--
  pages/continuity-steward/MyTasks.vue — CS open tasks.
-->
<template>
  <AppLayout>
    <AegisHeroBanner
    ...
    <AegisCard title="Tasks">
      <ul v-if="tasks.length" class="task-list">
        <li v-for="t in tasks" :key="t.id" class="task-row" :class="{ 'is-done': t.completed_at }">
          <button type="button" class="task-checkbox" :aria-label="t.completed_at ? 'Mark incomplete' : 'Mark complete'" @click="toggle(t)">
      ...
      <AegisEmptyState v-else icon="tasks" title="No tasks right now" description="Tasks assigned to you will appear here." />
    </AegisCard>
  </AppLayout>
</template>
...
function toggle(t) { router.post(route('cs.tasks.toggle', { task: t.id }), {}, { preserveScroll: true }) }
</script>
