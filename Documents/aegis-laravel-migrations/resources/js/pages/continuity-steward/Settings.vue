<!--
  pages/continuity-steward/Settings.vue — CS account preferences.
-->
<template>
  <AppLayout>
    <AegisHeroBanner eyebrow="Account" title="Settings" subtitle="Login, notifications, and stewardship preferences." />
    ...
    <AegisCard title="Stewardship preferences">
      <AegisToggle v-model="prefForm.accepting_new" label="Accepting new designations" />
      <AegisToggle v-model="prefForm.after_hours_alerts" label="After-hours critical incident alerts" />
      <AegisToggle v-model="prefForm.weekly_digest" label="Weekly summary digest" />
...
function saveAccount() { accountForm.put(route('cs.settings.account'), { preserveScroll: true, onSuccess: () => toast.success('Account updated.') }) }
function savePrefs()   { prefForm.put(route('cs.settings.prefs'),       { preserveScroll: true, onSuccess: () => toast.success('Preferences updated.') }) }
</script>
