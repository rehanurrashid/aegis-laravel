...
**Notes:** Sparse attribute table. The `notify_*` boolean gate keys (~26 of them) are read by `NotificationService` to decide whether to send each email type. Provides typed accessor: `cast(): mixed` based on `meta_type`.
